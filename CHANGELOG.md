...
- **The Docs Model has an API and CLI reference.** A new extractor (`parsers::api`) reads each Rust source file's documented surface — signatures, doc comments (summary, `# Errors`/`# Panics`/`# Examples` sections, examples with rustdoc's hidden lines removed, intra-doc links), visibility, fields, variants, trait and `impl` members, deprecation — and caches it by content hash in `.reflex/pulse/api.db` (this repo: 125 files in 172 ms cold, then free). A surface resolver walks each crate from `Cargo.toml` through `mod` declarations, decides what a user can reach (`pub` all the way down, `#[doc(hidden)]` excluded, `pub use` re-exports inlined like rustdoc), and attaches `impl` members to their types. The Docs tab gets one page per public module and per public type, with anchored symbols and working intra-doc links, plus one page per command read from clap's derive attributes (usage, arguments, options with defaults, subcommands). `pulse.toml` at the repository root (committable) or `[pulse.docs]` sets `library = false` for CLI-first projects and `include = ["crate::module"]`. The extraction does not run during `rfx index`.

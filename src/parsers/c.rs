...
pub fn parse(path: &str, source: &str) -> Result<Vec<SearchResult>> {
    ...
    symbols.extend(extract_unions(source, &table)?);
    symbols.extend(extract_typedefs(source, &table)?);
...
fn extract_unions(
    ...
    extract_symbols(source, table, 3, SymbolKind::Type, None)
}
...
fn extract_macros(
    ...
    extract_symbols(source, table, 6, SymbolKind::Macro, None)
}
